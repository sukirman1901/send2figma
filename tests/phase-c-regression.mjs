...
import { applyComponentNames, buildDesignSystemExport, summarizeDesignSystem, toDesignSystemJsonPayload } from "../extension/src/designSystem.js";
...
function testExport() {
  ...
  assert.match(exp.markdown, /# Fixture — Style Reference/);
  assert.match(exp.markdown, /Tokens — Colors/);
  assert.match(exp.markdown, /Tokens — Typography/);
  assert.match(exp.markdown, /Agent Prompt Guide/);
  assert.match(exp.markdown, /Do's and Don'ts|### Do/);
...
function testSurfaces() {
  assert.match(readFileSync(join(__dirname, "../extension/designSystemCapture.js"), "utf8"), /extractDesignSystem/);
  assert.match(readFileSync(join(__dirname, "../extension/background.js"), "utf8"), /htfy_EXTRACT_DESIGN_SYSTEM/);
  assert.match(readFileSync(join(__dirname, "../extension/ui/panel.js"), "utf8"), /extractDesignSystemFromPage|Extract from page/);
  assert.match(readFileSync(join(__dirname, "../extension/ui/panel.js"), "utf8"), /downloadDesignSystemJson|Export \.json/);
  assert.match(readFileSync(join(__dirname, "../extension/ui/panel.js"), "utf8"), /downloadDesignSystemMd|Export \.md/);
  assert.match(readFileSync(join(__dirname, "../extension/designSystemCapture.js"), "utf8"), /extractDesignSystem/);
  assert.match(readFileSync(join(__dirname, "../extension/src/designSystemMd.js"), "utf8"), /Token hierarchy/);
  assert.match(readFileSync(join(__dirname, "../extension/designSystemCapture.js"), "utf8"), /harvestCssVariables|fontFamilies/);
  assert.match(readFileSync(join(__dirname, "../figma-plugin/code.js"), "utf8"), /JSON has no tokens/);
  assert.match(readFileSync(join(__dirname, "../extension/src/fidelityPost.js"), "utf8"), /extractDesignTokensLive/);
  assert.match(readFileSync(join(__dirname, "../extension/src/fidelityPost.js"), "utf8"), /applyComponentNames/);
}
